...
var _Sources = (() => {
  ...
  var parseTiles = ($2) => {
    ...
    for (const element of $2("#book_list div.item[data-genre]").toArray()) {
  ...
  var countCards = ($2) => {
    return $2("#book_list div.item[data-genre]").length;
  };
  var isLastPage = ($2) => {
    return countCards($2) < MK_PAGE_SIZE;
  };
  var parseMangaDetails = ($2, mangaId) => {
    const title = $2("h1.heading").first().text().trim() || $2("h1").first().text().trim() || mangaId;
    const titles = [title];
    for (const alt of $2(".alt_name").first().text().split(";")) {
      const name = alt.trim();
      if (name.length > 0 && !titles.includes(name)) titles.push(name);
    }
    const image = ($2(".cover img").first().attr("src") ?? "").trim();
    const rawStatus = $2(".value.status").first().text().trim().toLowerCase();
    const status = rawStatus.includes("complet") ? "Completed" : "Ongoing";
    const author = $2(".authors a.author").toArray().map((element) => $2(element).text().trim()).filter((name) => name.length > 0).join(", ");
    const description = $2(".summary p").text().trim() || $2(".summary").text().replace(/^\s*Description\s*/i, "").trim();
    const tags = [];
    const seen = /* @__PURE__ */ new Set();
    for (const element of $2('ul.meta .genres a[href*="/genre/"], .info .genres a[href*="/genre/"]').toArray()) {
      const slug = /\/genre\/([a-z0-9-]+)\/?$/.exec($2(element).attr("href") ?? "")?.[1];
    ...
    return App.createSourceManga({
      ...
      mangaInfo: App.createMangaInfo({
        ...
        status,
        author,
        tags: tags.length > 0 ? [App.createTagSection({ id: "genre", label: "Genres", tags })] : []
      })
    });
  };
  var MONTHS = [
  ...
  var parseChapters = ($2) => {
    const rows = [];
    const seen = /* @__PURE__ */ new Set();
    for (const element of $2(".chapters tr").toArray()) {
      const row = $2(element);
  ...
  var parsePages = (html3) => {
    let best = [];
    for (const match of html3.matchAll(/=\s*\[([^\]]+)\]/g)) {
      const urls = [...match[1].matchAll(/['"](https?:\/\/[^'"]+)['"]/g)].map((entry) => entry[1].replace(/^http:\/\//, "https://")).filter((url) => !url.endsWith(".js") && !url.endsWith(".css"));
...
Please go to the homepage of <${MangaKatanaInfo.name}> and press the cloud icon.`);
    ...
    async getHomePageSections(sectionCallback) {
      for (const entry of SECTIONS) {
        ...
        const $2 = await this.loadPage(filteredListingUrl(entry.order, 1));
        section.items = this.tilesFrom(parseTiles($2), /* @__PURE__ */ new Set());
