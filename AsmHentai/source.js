...
var _Sources = (() => {
  ...
  var AsmHentai_exports = {};
  __export(AsmHentai_exports, {
    ASM_DOMAIN: () => ASM_DOMAIN,
    AsmHentai: () => AsmHentai,
    AsmHentaiInfo: () => AsmHentaiInfo,
  ...
  var AsmHentai = class {
    ...
    getMangaShareUrl(mangaId) {
      return isSeriesId(mangaId) ? `${ASM_DOMAIN}/search/?q=${encodeURIComponent(baseFromSeriesId(mangaId))}` : `${ASM_DOMAIN}/g/${mangaId}/`;
    }
...
Please go to the homepage of <${AsmHentaiInfo.name}> and press the cloud icon.`);
    ...
    async pagedListing(path, page, seen, filters2) {
      ...
      for (let hop = 0; hop < 5; hop++) {
        const $2 = await this.loadPage(this.pagedUrl(path, current));
    ...
    async volumesOf(base) {
      ...
      if (found.size > 0) {
        try {
          const first2 = orderVolumes(Array.from(found.values()))[0];
          const $2 = await this.loadPage(`${ASM_DOMAIN}/g/${first2.id}/`);
          const artists = this.metaRow($2, "Artists");
    ...
    async representativeId(mangaId) {
      ...
      return (await this.volumesOf(baseFromSeriesId(mangaId)))[0].id;
    }
    /**
    ...
    metaRow($2, label) {
      const values = [];
      for (const element of $2("h3").toArray()) {
        const heading = $2(element);
        ...
        for (const link of heading.nextAll(".tag_list").first().find("a").toArray()) {
          ...
          const name = $2(link).text().replace(/\s*\([\d,]+\)\s*$/, "").replace(/\s+/g, " ").trim();
          if (slug != void 0 && name.length > 0) values.push({ slug, name });
        }
        break;
      }
      return values;
    ...
    async getMangaDetails(mangaId) {
      ...
      const $2 = await this.loadPage(`${ASM_DOMAIN}/g/${galleryId}/`);
      this.guard($2);
      const raw = $2("h1").first().text().replace(/\s+/g, " ").trim();
      const japanese = $2("h2").first().text().replace(/\s+/g, " ").trim();
      ...
      let cover = ($2("img.lazy, .cover img").first().attr("data-src") ?? $2(".cover img").first().attr("src") ?? "").trim();
      if (cover.startsWith("//")) cover = `https:${cover}`;
      const tags = [];
      const seen = /* @__PURE__ */ new Set();
      for (const [type, label] of [["tag", "Tags"], ["artist", "Artists"], ["parody", "Parodies"], ["category", "Categor"]]) {
        for (const entry of this.metaRow($2, label)) {
          const id = `${type}:${entry.slug}`;
          if (seen.has(id)) continue;
          seen.add(id);
          tags.push(App.createTag({ id, label: entry.name }));
        }
      }
      const artists = this.metaRow($2, "Artists").map((entry) => entry.name);
      const pages = /Pages:\s*(\d+)/i.exec($2.html() ?? "")?.[1];
      return App.createSourceManga({
        id: mangaId,
        mangaInfo: App.createMangaInfo({
          titles,
          image: cover,
          desc: pages != void 0 ? `${pages} pages.` : "",
          status: "Completed",
          author: artists.join(", "),
    ...
    async getChapters(mangaId) {
      if (!isSeriesId(mangaId)) {
        const $2 = await this.loadPage(`${ASM_DOMAIN}/g/${mangaId}/`);
        this.guard($2);
        const raw = $2("h1").first().text().replace(/\s+/g, " ").trim();
    ...
    async getChapterDetails(mangaId, chapterId) {
      ...
      const html3 = await this.fetchHtml(`${ASM_DOMAIN}/g/${galleryId}/`);
      const $2 = load(html3);
      this.guard($2);
      const byNumber = /* @__PURE__ */ new Map();
      let shard = "";
      let extension = "jpg";
      for (const match of html3.matchAll(/\/\/images\.asmhentai\.com\/(\d+)\/(\d+)\/(\d+)t\.(\w+)/g)) {
        if (match[2] !== galleryId) continue;
        shard = match[1];
        extension = match[4];
        byNumber.set(Number(match[3]), `https://images.asmhentai.com/${shard}/${galleryId}/${match[3]}.${extension}`);
      }
      const declared = Number(/Pages:\s*(\d+)/i.exec(html3)?.[1] ?? "0");
    ...
    async getHomePageSections(sectionCallback) {
      for (const entry of SECTIONS) {
        ...
        const $2 = await this.loadPage(this.pagedUrl(entry.path, 1));
        section.items = this.tilesFrom(this.parseCards($2), /* @__PURE__ */ new Set());
  ...
  return __toCommonJS(AsmHentai_exports);
})();
