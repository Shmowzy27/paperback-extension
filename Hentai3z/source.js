...
var _Sources = (() => {
  ...
  // src/Hentai3z/Hentai3z.ts
  var Hentai3z_exports = {};
  __export(Hentai3z_exports, {
    H3Z_DOMAIN: () => H3Z_DOMAIN,
    Hentai3z: () => Hentai3z,
    Hentai3zInfo: () => Hentai3zInfo,
  ...
  var cleanTitle = (raw) => {
    let text3 = raw;
    let previous = "";
    while (previous !== text3) {
      previous = text3;
      text3 = text3.replace(/[\[(（][^\[\]()（）]*[\])）]/g, "");
    }
    const halves = text3.split("|").map((half) => half.trim()).filter((half) => half.length > 0);
    text3 = halves.length > 0 ? halves[0] : text3;
    return text3.replace(/\s+/g, " ").trim().replace(/^[-~:.\s]+|[-~:.\s]+$/g, "");
  };
  ...
  var sharesLead = (a, b) => {
    ...
    return distinctive(short) && short.every((word, index2) => long[index2] === word);
  };
  var sharesTail = (a, b) => {
    const x = keyWords(a);
    const y = keyWords(b);
  ...
  var bookRoot = (clean) => {
    ...
    const root2 = seriesKey(clean.slice(0, cut));
    return distinctive(root2.split(" ").filter((word) => word.length > 0)) ? root2 : "";
  };
  ...
  var foldTiles = (items, seen, memo) => {
    ...
    for (const item of items) {
      ...
      const root2 = book ? bookRoot(clean) : "";
      if (root2.length > 0) {
        if (seen.has(`r:${root2}`)) continue;
        seen.add(`r:${root2}`);
      }
      let folded = false;
      for (const other of series) {
        ...
        if (key.length > other.key.length && book && !other.book) {
          ...
          folded = root2.length > 0 && bookRoot(other.clean) === root2;
        }
        if (folded) {
  ...
  var orderVolumes = (volumes) => {
    ...
    if (ordered.length > 1 && ordered.every((volume) => volume.numbered === false)) {
      ...
      ordered.forEach((volume, index2) => {
        volume.volume = index2 + 1;
      });
      return ordered;
    }
    let next2 = ordered.filter((volume) => volume.volume < FINAL).reduce((highest, volume) => Math.max(highest, Math.floor(volume.volume)), 0) + 1;
    for (const volume of ordered) {
      if (volume.volume >= FINAL) volume.volume = next2++;
    }
    return ordered;
  };
  ...
  // src/Hentai3z/ParodyNames.ts
  var PARODY_PHRASES = ["7th dragon", "abigail williams", "accel world", "ace attorney", "aerith gainsborough", "aether", "agrias oaks", "ah my goddess", "ai amatsu", "ai chan", "ai haibara", "ai hinatsuru", "ai miyashita", "ai mizuno", "ai nanasaki", "aichi sendou", "aikatsu", "aiko senoo", "aiko takamori", "airi totoki", "akane hino", "akane kotonoha", "akane murokasa", "akane shinjou", "akane tendo", "akari kamigishi", "akari kanzaki", "akari kizuna", "akazukin cha cha", "akebi no mi", "akebono", "akeno himejima", "aki hinata", "aki izayoi", "akigumo", "akiha tohno", "akihisa yoshii", "akiko minase", "akira kurusu", "akitsu maru", "akizuki", "ako amau", "ako atarashi", "ako shirabe", "albedo", "aldnoah zero", "alexis rhodes", "alhaitham", "alibaba saluja", "alice gear aegis", "alice in wonderland", "alice liddell", "alice margatroid", "alice tendou", "alisa ilinichina amiella", "alisa reinford", "amagami", "amagi brilliant park", "amamiya kokoro", "amana osaki", "amane kanata", "amate yuzuriha", "amatsukaze", "amelia wil tesla seyruun", "ami futami", "ami mizuno", "ana coppola", "anastasia nikolaevna romanova", "andira", "android 18", "angel beats", "angela balzac", "angelic layer", "anko mitarashi", "anna mochizuki", "anna yamada", "anne takamaki", "annie leonhart", "ano hi mita hana no namae wo bokutachi wa mada shiranai", "ano natsu de matteru", "ansatsu kyoushitsu", "anthy himemiya", "anya forger", "anyamaru tantei kiruminzoo", "anzu futaba", "anzu kadotani", "ao no exorcist", "aoba seragaki", "aoba suzukaze", "aoba utsumi", "aoi inuyama", "aoi kiriya", "aoi kotonoha", "aoi oki", "aoko aozaki", "aqua hoshino", "aquarion evol", "ar tonelico", "arashio", "arataka reigen", "arcana heart", "arcueid brunestud", "aria von reiji asuna", "arika yumemiya", "arisa ichigaya", "arisu shimada", "arisu tachibana", "arknights", "arknights endfield", "arlecchino", "armin arlert", "arpeggio of blue steel", "artoria caster", "artoria pendragon", "artoria pendragon alter", "aru rikuhachima", "arulumaya", "asagi igawa", "asashio", "ash ketchum", "ashelia bnargin dalmasca", "ashigara", "ashita no nadja", "astarotte ygvar", "astolfo", "astra yao", "asuka langley soryu", "asuna ichinose", "asuna kagurazaka", "asuna yuuki", "ataru moroboshi", "atelier ryza", "athena asamiya", "athrun zala", "atsuko hakari", "axis powers hetalia", "aya shameimaru", "aya toujou", "ayaka kamisato", "ayaka kurusugawa", "ayanami", "ayane okusora", "ayase aragaki", "ayeka masaki jurai", "ayumu uehara", "azumanga daioh", "azur lane", "azusa miura", "azusa nakano", "azusa shirasu", "baiken", "baka to test to shoukanjuu", "bakemonogatari", "bakuman", "bakusou kyoudai lets and go", "bamboo blade", "bang dream", "baobhan sith", "barbara pegg", "barghest", "barnaby brooks jr", "battle athletes", "battle spirits", "bayonetta", "bazett fraga mcremitz", "beat angel escalayer", "beatmania", "beatrix", "bell cranel", "belldandy", "betterman", "bianca whitaker", "big hero 6", "black butler", "black cat", "black lagoon", "blazblue", "bleach", "blend s", "blue archive", "blue lock", "blue mary", "boa hancock", "bocchi the rock", "boku dake ga inai machi", "boku no kokoro no yabai yatsu", "boku wa tomodachi ga sukunai", "bokutachi wa benkyou ga dekinai", "bomber girl", "boruto", "boruto uzumaki", "bremerton", "bridget", "bulma briefs", "busou renkin", "byakuren hijiri", "byleth eisner", "c c", "caelus", "caenis", "cagalli yula athha", "cagliostro", "cammy white", "captain nemo", "cardcaptor sakura", "cardfight vanguard", "cecilia alcott", "chacha", "chainsaw man", "charlotte dunois", "charlotte e yeager", "chi chi", "chiaki morisawa", "chibiusa", "chie sasaki", "chie satonaka", "chieri ogata", "chifuyu matsuno", "chihaya kisaragi", "chihiro senkawa", "chiho sasaki", "chika fujiwara", "chika itou", "chika takami", "china kousaka", "chinatsu hinomiya", "chino kafuu", "chisato nishikigi", "chitoge kirisaki", "chiya ujimatsu", "chiyo mihama", "chiyo shimada", "chiyomi anzai", "chiyuki kuwayama", "chizuru ichinose", "chloe von einzbern", "chobits", "choromatsu matsuno", "choukai", "chousoku henkei gyrozetter", "chris yukine", "chun li", "chuunibyou demo koi ga shitai", "ciel phantomhive", "cinque izumi", "citlali", "city hunter", "clannad", "cloud strife", "clownpiece", "code geass", "comic party", "conan edogawa", "cornelia li britannia", "cosmic baton girl comet san", "creamy mami", "cure ange", "cure arcana shadow", "cure beauty", "cure black", "cure chocolat", "cure diamond", "cure heart", "cure honey", "cure macaron", "cure macherie", "cure magical", "cure march", "cure melody", "cure milky", "cure miracle", "cure moonlight", "cure peace", "cure prism", "cure sky", "cure star", "cure sunny", "cure sunshine", "cure sword", "cure twinkle", "cure white", "cutey honey", "d gray man", "da capo", "dagashi kashi", "daiki aomine", "daiya no ace", "daiyousei", "dandadan", "danganronpa", "darker than black", "darkstalkers", "darling in the franxx", "date a live", "dead or alive", "death note", "debora briscoletti", "deedlit", "delicious party precure", "dennou coil", "detective conan", "detective conan meitantei conan", "devil may cry", "di gi charat", "dia kurosawa", "digimon", "digimon adventure", "digimon frontier", "digimon tamers", "dirty pair", "disgaea", "disney twisted wonderland", "djeeta", "dog days", "dokidoki precure", "doppo kannonzaka", "doraemon", "doremi harukaze", "doremy sweet", "doutanuki masakuni", "dr stone", "dragon ball", "dragon ball gt", "dragon ball super", "dragon ball z", "dragon quest", "dragon quest dai no daibouken", "dragon quest ii", "dragon quest iii", "dragon quest iv", "dragon quest v", "dragon quest viii", "dragon quest xi", "dragons crown", "dramatical murder", "dream c club", "duel masters", "dungeon meshi", "dungeon ni deai o motomeru no wa machigatteiru darou ka", "duo maxwell", "durarara", "dynasty warriors", "edmond dantes", "edward elric", "eijirou kirishima", "eijun sawamura", "eila ilmatar juutilainen", "eirin yagokoro", "el hazard", "elena shimabara", "elf yamada", "eli ayase", "elizabeth bathory", "ellen joe", "elpeo ple", "elsword", "emma verde", "ena ayase", "ensemble stars", "eren jaeger", "ereshkigal", "eri ayase", "eri sawachika", "eri shirao", "erica hartmann", "erich von rerugen", "erika itsumi", "erika kurumi", "eriko futami", "erina nakiri", "erio mondial", "eriri spencer sawamura", "eris boreas greyrat", "ero trap dungeon", "eromanga sensei", "erstin ho", "eru chitanda", "erwin smith", "etorofu", "etrian odyssey", "eula lawrence", "euphemia li britannia", "eureka 7", "evangeline a k mcdowell", "evelyn chevalier", "expelled from paradise", "exusiai", "eyeshield 21", "fairy tail", "fatal fury", "fate apocrypha", "fate extra", "fate grand order", "fate hollow ataraxia", "fate kaleid liner prisma illya", "fate stay night", "fate testarossa", "fate zero", "final fantasy", "final fantasy iv", "final fantasy ix", "final fantasy tactics", "final fantasy v", "final fantasy vi", "final fantasy vii", "final fantasy viii", "final fantasy x", "final fantasy x 2", "final fantasy xi", "final fantasy xii", "final fantasy xiii", "final fantasy xiv", "final fantasy xv", "final fight", "fire emblem", "fire emblem awakening", "fire emblem if", "fire emblem mystery of the emblem", "fire emblem three houses", "firefly", "fischl", "flandre scarlet", "flay allster", "flower knight girl", "focalors", "fourth raikage", "frame arms girl", "francesca lucchini",
ryuu", "historys strongest disciple kenichi", "hitagi senjougahara", "hitoha marui", "hitori gotou", "hiyori tsuchinaga", "hololive", "homura akemi", "hong meiling", "honkai gakuen", "honkai star rail", "honoka kosaka", "honoka kousaka", "honoka yukishiro", "hoozuki no reitetsu", "hoshikawa sara", "hoshimachi suisei", "hoshino takanashi", "hotaru shidare", "hotaru tomoe", "houki shinonono", "houshou", "houshou marine", "houtarou oreki", "hsien ko", "hu tao", "hugtto precure", "hunter x hunter", "hyouka", "hyperdimension neptunia", "hypnosis mic", "i 13", "i 19", "i 26", "i 401", "i 58", "i 8", "ibaraki douji", "ibuki douji", "ibuki tanga", "ichigo 100", "ichigo hoshimiya", "ichigo kurosaki", "ichigo mashimaro", "ichigo morino", "ichika nakamasa", "ichika nakano", "ichika orimura", "ichika takatsuki", "ichimatsu matsuno", "ichirin kumoi", "ichiro ogami", "ichiro yamada", "identity v", "idolish7", "idolmaster", "idolmaster sidem", "ika musume", "ikaruga", "ikkitousen", "iku nagae", "iku nakatani", "ikuyo hoshizora", "ikuyo kita", "illyasviel von einzbern", "inazuma eleven", "inazuma eleven go", "index librorum prohibitorum", "infinite stratos", "ino yamanaka", "inu x boku ss", "inugami korone", "inui toko", "inuyasha", "iori minase", "iori shiromi", "iori yoshizuki", "iris chateaubriand", "irisviel von einzbern", "iroha isshiki", "iroha natsume", "iroha tamaki", "isami hanaoka", "isokaze", "issei hyoudou", "isuzu sento", "its not my fault that im not popular", "itsuki nakano", "ivy valentine", "izaya orihara", "izuku midoriya", "izumi sena", "izuna kuda", "jack atlas", "jack the ripper", "jade curtiss", "jaden yuki", "jane doe", "jean gunnhildr", "jean kirschtein", "jeanne alter", "jeanne darc", "jessica albert", "jigoku no fubuki", "jill valentine", "jin ping mei", "jingei", "jinpachi toudou", "jintsuu", "jojos bizarre adventure", "joon yorigami", "jotaro kujo", "jtf mob", "jujutsu kaisen", "jun sagami", "junichi tachibana", "junko enoshima", "junko konno", "juri han", "juushimatsu matsuno", "k on", "kaede isami", "kaede takagaki", "kagami hiiragi", "kagerou imaizumi", "kagome higurashi", "kaguya houraisan", "kaguya sama wa kokurasetai", "kaguya shinomiya", "kaho hinoshita", "kaho komiya", "kaito kuroba", "kaitou tenshi twin angel", "kakashi hatake", "kakegurui", "kako takafuji", "kallen stadtfeld", "kaltsit", "kana arima", "kanade hayami", "kanade minamino", "kanade tachibana", "kanako mimura", "kanako ohno", "kanako yasaka", "kaname chidori", "kanan matsuura", "kanao tsuyuri", "kanata konoe", "kanna kamui", "kanna ogata", "kannagi", "kanoe itagaki", "kanojo okarishimasu", "kanon", "kantai collection", "kanu unchou", "kaolla su", "kaori kanzaki", "kaori nishino", "kaori sakuramori", "kaoru kamiya", "kaoru niimi", "kaoru ryuzaki", "kaoruko sazaki", "karakai jouzu no takagi san", "karamatsu matsuno", "karen araragi", "karen hojo", "karen onodera", "kari kamiya", "karin asaka", "karin kakudate", "karin kanzuki", "kasen ibara", "kashima", "kash
ie kisaragi", "yugi mutou", "yui hirasawa", "yui kotegawa", "yui kusano", "yui ohtsuki", "yui yuigahama", "yukari akiyama", "yukari kadenokouji", "yukari takeba", "yukari yakumo", "yukari yuzuki", "yuki himekawa", "yuki mori", "yuki nagato", "yukihana lamy", "yukiho hagiwara", "yukikaze mizuki", "yukikaze panettone", "yukiko amagi", "yukimi sajo", "yukino yukinoshita", "yukio okumura", "yuko yoshida", "yuma tsukumo", "yunyun", "yuri lowell", "yuri on ice", "yuri sakazaki", "yurika misumaru", "yuriko nanao", "yuru camp", "yuruyuri", "yusei fudo", "yuu morisawa", "yuu narukami", "yuubari", "yuudachi", "yuugi hoshiguma", "yuugumo", "yuuichi aizawa", "yuuji itadori", "yuuka hayase", "yuuka kazami", "yuuki otokura", "yuuki tachimukai", "yuuki yuuna wa yuusha de aru", "yuuno scrya", "yuuri katsuki", "yuusaku fujiki", "yuuta togashi", "yuuya sakaki", "yuyuko saigyouji", "yuyushiki", "yuzu hanaoka", "yuzuki choco", "zapp renfro", "zenitsu agatsuma", "zenless zone zero", "zero no tsukaima", "zero two", "zeta gundam", "zettai junpaku mahou shoujo", "zettai karen children", "zhongli", "zhu yuan", "zoids genesis", "zombie land saga", "zuihou", "zuikaku"];

  // src/Hentai3z/Hentai3z.ts
  var H3Z_DOMAIN = "https://hentai3z.cc";
  ...
  var SITE_BANNED_LABELS = /\btrap\b|\bfemboy\b|\botokonoko\b/i;
  var META_TAGS = /* @__PURE__ */ new Set([
    "already uploaded",
    "missing cover",
    "out of order",
    "sample",
    ...
    "full censorship",
    "mosaic censorship",
    "replace with short hair",
    "property tag",
    "ganari ryuu",
    "ruko kominato",
    "soul calibur"
  ]);
  var UNCENSORED = /\buncensored\b/i;
  var twinKey = (title) => seriesKey(title.replace(/\(\s*(?:official\s+)?(?:un)?censored\s*\)/gi, " ").replace(/\b(?:official\s+)?(?:un)?censored\b/gi, " "));
  var PARODY_TAG_SLUGS = /* @__PURE__ */ new Set(["anime", "game"]);
  var NOT_A_SERIES = /^(?:c\d+|comic\b|comitia|reitaisai|sc\d+|\d|vol|ch\b|chapter|part|english|uncensored|decensored|colou?ri[sz]ed|digital|josei|full colou?r|ongoing|complete)/i;
  var normalise = (text3) => " " + text3.toLowerCase().normalize("NFKD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-z0-9]+/g, " ").trim() + " ";
  var parodyIn = (title) => {
    const western = /\(([^()]+)\)\s*\[[^\]]+\]\s*$/.exec(title);
    if (western != void 0 && !NOT_A_SERIES.test(western[1].trim())) return western[1].trim();
    const text3 = normalise(title);
    return PARODY_PHRASES.find((phrase) => text3.includes(` ${phrase} `));
  };
  var MULTI_WORK_SLUG = "multi-work-series";
  var NOT_ENGLISH = /^(?:raw|chinese|japanese|korean|spanish|thai|vietnamese|indonesian|french|german|portuguese|russian|italian|polish|arabic)\b/i;
  var SECTIONS = [
  ...
  var slugOf = (href) => /\/(?:hentai|comic)\/([^/?#"]+)\/?$/.exec(href)?.[1];
  var isSerial = (inspection) => inspection.chapters.length > 1 || inspection.chapters.some((chapter) => /^chapter-/i.test(chapter.slug));
  var chapterNumber = (slug, name) => {
    const match = /chapter\s*[- ]?(\d+(?:\.\d+)?)\s*-?\s*([a-i])?\b/i.exec(name) ?? /^(?:chapter-)?(\d+(?:\.\d+)?)(?:-([a-i]))?$/i.exec(slug);
    if (match == void 0) return NaN;
    const part = match[2] != void 0 ? (match[2].toLowerCase().charCodeAt(0) - 96) / 10 : 0;
    return Number(match[1]) + part;
  };
  var englishChapters = (inspection) => inspection.chapters.filter((chapter) => !NOT_ENGLISH.test(chapter.slug) && !NOT_ENGLISH.test(chapter.name));
  var creditsOf2 = (inspection) => {
  ...
  var titleRefusal = (title) => BANNED_LABELS.test(title) || parodyIn(title) != void 0;
  var Hentai3zInfo = {
    version: "1.0.1",
    name: "Hentai3z",
    icon: "icon.png",
  ...
  var Hentai3z = class {
    ...
    getMangaShareUrl(mangaId) {
      return isSeriesId(mangaId) ? `${H3Z_DOMAIN}/list-manga?search=${encodeURIComponent(baseFromSeriesId(mangaId))}` : `${H3Z_DOMAIN}/hentai/${mangaId}`;
    }
...
Please go to the homepage of <${Hentai3zInfo.name}> and press the cloud icon.`);
    ...
    async fetchHtml(url) {
      ...
      if (/<title>\s*Just a moment/i.test(html3)) throw this.cloudflareError();
      return html3;
    }
    async loadPage(url) {
    ...
    listingUrl(listing, page) {
      if (listing.genre != void 0) {
        return page <= 1 ? `${H3Z_DOMAIN}/manga-list/${listing.genre}` : `${H3Z_DOMAIN}/manga-list/${listing.genre}/${page}`;
      }
      const path = page <= 1 ? "/list-manga" : `/list-manga/${page}`;
      const query = listing.search != void 0 ? `search=${encodeURIComponent(listing.search)}` : `order_by=${listing.sort ?? "lastest"}`;
    ...
    parseCards(html3) {
      ...
      for (const element of $2("div.story_item").toArray()) {
        const item = $2(element);
        const anchor = item.find(".mg_name a").first();
        const slug = slugOf(anchor.attr("href") ?? "");
        const title = anchor.text().replace(/\s+/g, " ").trim();
        if (slug == void 0 || title.length === 0 || seen.has(slug)) continue;
        seen.add(slug);
        cards.push({ slug, title, thumb: absolute(item.find(".story_images img").first().attr("src") ?? "") });
      }
      return cards;
    }
    /**
     ...
     */
    async inspect(slug) {
      const cached = this.remembered(`d:${slug}`);
      if (cached != void 0) return cached;
      const $2 = await this.loadPage(`${H3Z_DOMAIN}/hentai/${slug}`);
      const title = ($2(".detail_name h1").first().text() || $2("h1").first().text()).replace(/\s+/g, " ").trim() || slug;
      const rows = $2(".detail_listInfo .item").toArray().map((element) => $2(element));
      const valueOf = (label) => rows.find((row) => row.find(".info_label").text().trim().toLowerCase().startsWith(label))?.find(".info_value");
      const artists = (valueOf("artist")?.find("a").toArray().map((element) => $2(element).text().trim()) ?? []).filter((name) => name.length > 0 && !/^updating$/i.test(name));
      const author = (valueOf("author")?.text() ?? "").replace(/\s+/g, " ").trim();
      const genres = [];
      for (const element of $2('.detail_listInfo a[href*="/manga-list/"]').toArray()) {
        const genre = /\/manga-list\/([^/?#"]+)\/?$/.exec($2(element).attr("href") ?? "")?.[1];
        const name = $2(element).text().replace(/\s+/g, " ").trim();
        if (genre != void 0 && name.length > 0 && !genres.some((known) => known.slug === genre)) genres.push({ slug: genre, name });
      }
      let desc = "";
      for (const element of $2(".detail_review").toArray()) {
        const block = $2(element);
      ...
      const chapters = [];
      for (const element of $2("a.chapter_num").toArray()) {
        const anchor = $2(element);
        const chapter = new RegExp(`/(?:hentai|comic)/${slug.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}/([^/?#"]+)`).exec(anchor.attr("href") ?? "")?.[1];
        if (chapter == void 0 || chapters.some((known) => known.slug === chapter)) continue;
        ...
        const time = date != void 0 ? new Date(Number(date[3]), Number(date[2]) - 1, Number(date[1])).getTime() : 0;
        chapters.push({ slug: chapter, name: name.length > 0 ? name : chapter, number, time });
      }
      const inspection = {
        slug,
        title,
    ...
    refusalOf(inspection) {
      ...
      if (banned != void 0) return `This title is filed under "${banned}", which is excluded by your settings, and will not be shown.`;
      if (groupRefusal(labels) != void 0) return GROUP_REFUSAL_MESSAGE;
    ...
    async gather(url) {
      ...
      const candidates = await Promise.all(cards.map(async (card) => {
        if (titleRefusal(card.title)) return { card, refused: true };
        try {
          const inspection = await this.inspect(card.slug);
          return { card, inspection, refused: inspection.refusal != void 0 };
        } catch {
          return { card, refused: true };
        }
      }));
      return { cards: cards.length, candidates };
    }
    matches(inspection, filters2) {
      if (filters2 == void 0) return true;
      const slugs = inspection.genres.map((genre) => genre.slug);
      if (filters2.exclude.some((slug) => slugs.includes(slug))) return false;
      return filters2.include.every((slug) => slugs.includes(slug));
    }
    /**
     ...
     */
    tilesFrom(candidates, seen, serials, filters2) {
      const tiles = [];
      let run = [];
      const flush = () => {
      ...
      const shown = (candidate) => {
        ...
        if (candidate.refused || inspection == void 0) return void 0;
        if (inspection.chapters.length === 0 || !this.matches(inspection, filters2)) return void 0;
        return inspection;
      };
      const preferred = /* @__PURE__ */ new Map();
      for (const candidate of candidates) {
        const inspection = shown(candidate);
        if (inspection == void 0 || !isSerial(inspection)) continue;
        const key = twinKey(inspection.title);
        const current = preferred.get(key);
        if (current == void 0 || UNCENSORED.test(inspection.title) && !UNCENSORED.test(current.title)) preferred.set(key, inspection);
      }
      for (const candidate of candidates) {
        const inspection = shown(candidate);
        if (inspection == void 0) continue;
        ...
        run.push({
          ...
          creators: creditsOf2(inspection),
          payload: inspection
        });
      }
      flush();
     ...
     */
    async pagedListing(listing, metadata, filters2) {
      ...
      for (let hop = 0; hop < 3; hop++) {
        ...
        tiles.push(...this.tilesFrom(page.candidates, seen, serials, filters2));
        current++;
        if (tiles.length >= 8) break;
      }
      return App.createPagedResults({
        results: tiles,
        metadata: exhausted ? void 0 : { page: current, seen: Array.from(seen), serials: Array.from(serials) }
      });
    }
    /**
    ...
    async volumesOf(base) {
      ...
      const consider = (rows, sameArtist, trusted = false) => {
        for (const row of rows) {
          ...
          if (inspection != void 0 && isSerial(inspection)) continue;
          const credited = sameArtist && inspection != void 0 && creditsOf2(inspection).some((name) => credits.includes(name));
          const verdict = volumeOf(asCandidate(row), base, longName, credited, trusted, members);
          if (!verdict.belongs) continue;
          const chapter = inspection != void 0 ? englishChapters(inspection)[0] : void 0;
          if (row.refused || inspection == void 0 || chapter == void 0) {
            refused++;
            continue;
          }
    ...
    async getMangaDetails(mangaId) {
      ...
      if (isSeriesId(mangaId)) {
        ...
        inspection = await this.inspect((await this.volumesOf(base))[0].slug);
        title = base;
      } else {
        inspection = await this.inspect(mangaId);
        if (inspection.refusal != void 0) throw new Error(inspection.refusal);
      ...
      const tags = inspection.genres.map((genre) => App.createTag({ id: genre.slug, label: genre.name }));
    ...
    async getChapters(mangaId) {
      if (isSeriesId(mangaId)) {
        ...
        return volumes.map((volume, index2) => App.createChapter({
      ...
      if (inspection.refusal != void 0) throw new Error(inspection.refusal);
      const rows = englishChapters(inspection).map((chapter, index2) => ({ chapter, order: -index2 }));
      rows.sort((a, b) => !isNaN(a.chapter.number) && !isNaN(b.chapter.number) ? a.chapter.number - b.chapter.number : a.order - b.order);
      const serial = isSerial(inspection);
      return rows.map((row, index2) => App.createChapter({
        id: `${inspection.slug}/${row.chapter.slug}`,
        chapNum: isNaN(row.chapter.number) ? index2 + 1 : row.chapter.number,
    ...
    async getChapterDetails(mangaId, chapterId) {
      ...
      if (inspection.refusal != void 0) throw new Error(inspection.refusal);
      const html3 = await this.fetchHtml(`${H3Z_DOMAIN}/hentai/${slug}/${chapter}`);
      const body = /slides_p_path\s*=\s*\[([^\]]*)\]/.exec(html3)?.[1] ?? "";
      const pages = [];
     ...
     */
    async getSearchTags() {
      const cached = this.remembered("catalog");
      if (cached != void 0) return cached;
      const found = /* @__PURE__ */ new Map();
      try {
        const $2 = await this.loadPage(`${H3Z_DOMAIN}/list-manga`);
        for (const element of $2('a[href*="/manga-list/"]').toArray()) {
          const slug = /\/manga-list\/([^/?#"]+)\/?$/.exec($2(element).attr("href") ?? "")?.[1];
          const name = $2(element).text().replace(/\s+/g, " ").trim();
          if (slug == void 0 || name.length === 0 || found.has(slug)) continue;
          if (BANNED_LABELS.test(name) || TAG_ONLY_LABELS.test(name) || SITE_BANNED_LABELS.test(name)) continue;
          if (PARODY_TAG_SLUGS.has(slug) || parodyIn(name) != void 0 || META_TAGS.has(name.toLowerCase())) continue;
          found.set(slug, name);
        }
      } catch {
        return [];
      }
      const tags = [...found.entries()].sort((a, b) => a[1].localeCompare(b[1])).map(([slug, name]) => App.createTag({ id: slug, label: name }));
      const sections = tags.length > 0 ? [App.createTagSection({ id: "tags", label: "Tags", tags })] : [];
      if (sections.length > 0) this.remember("catalog", sections, 864e5);
      return sections;
    }
    async getHomePageSections(sectionCallback) {
      for (const entry of SECTIONS) {
        ...
        section.items = this.tilesFrom(page.candidates, /* @__PURE__ */ new Set(), /* @__PURE__ */ new Set());
  ...
  return __toCommonJS(Hentai3z_exports);
})();
