...
var _Sources = (() => {
  ...
  var HentaiNexus_exports = {};
  __export(HentaiNexus_exports, {
  ...
  // src/HentaiNexus/HentaiNexusDecoder.ts
  var HN_HOSTNAME = "hentainexus.com";
  ...
  function decodeReaderPayload(payload, hostname = HN_HOSTNAME) {
    const data2 = base64ToBytes(payload);
    if (data2.length <= 64) {
    ...
    for (let i2 = 0; i2 < keyLength; i2++) {
      data2[i2] = data2[i2] ^ hostname.charCodeAt(i2);
    }
    let crc = 0;
    for (let i2 = 0; i2 < 64; i2++) {
      crc ^= data2[i2];
      for (let bit = 0; bit < 8; bit++) {
        crc = crc & 1 ? crc >>> 1 ^ 12 : crc >>> 1;
      }
    }
    const stride = firstPrimes()[crc & 7];
    const state = new Uint8Array(256);
    for (let i2 = 0; i2 < 256; i2++) state[i2] = i2;
    let j = 0;
    for (let i2 = 0; i2 < 256; i2++) {
      j = (j + state[i2] + data2[i2 % 64]) % 256;
      const swap = state[i2];
      state[i2] = state[j];
      state[j] = swap;
    }
    ...
    j = 0;
    const decoded = [];
    for (let n = 0; n + 64 < data2.length; n++) {
      i = (i + stride) % 256;
      ...
      decoded.push(String.fromCharCode(data2[n + 64] ^ keyByte));
    }
  ...
  var detailText = ($2, label) => {
    const row = $2("table.view-page-details tr").filter((_, element) => $2(element).find("td.viewcolumn").text().trim() === label).first().find("td").not(".viewcolumn").first();
    return cleanText($2, row);
  };
  var tagFromHref = (href) => {
...
Please go to the homepage of <${HentaiNexusInfo.name}> and press the cloud icon.`);
  ...
  return __toCommonJS(HentaiNexus_exports);
})();
