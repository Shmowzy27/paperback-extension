...
var _Sources = (() => {
  ...
  var ManhwaClubAll_exports = {};
  __export(ManhwaClubAll_exports, {
    ManhwaClubAll: () => ManhwaClubAll,
    ManhwaClubAllInfo: () => ManhwaClubAllInfo
  });
  ...
  var parseMangaPostId = (html3) => {
    return /"manga_id"\s*:\s*"(\d+)"/.exec(html3)?.[1] ?? /postid-(\d+)/.exec(html3)?.[1] ?? /data-id="(\d+)"/.exec(html3)?.[1];
  };
  var STATUS_MAP = {
    "completed": "Completed",
    "complete": "Completed",
  ...
  var parseMangaDetails = ($2, mangaId) => {
    ...
    const rawStatus = detailText($2, "status").toLowerCase();
    const status = STATUS_MAP[rawStatus] ?? "Ongoing";
    const tags = [];
    const seen = /* @__PURE__ */ new Set();
    for (const element of $2('a[href*="/manga-genre/"]').toArray()) {
      const slug = /\/manga-genre\/([^/]+)\/?/.exec($2(element).attr("href") ?? "")?.[1];
    ...
    return App.createSourceManga({
      ...
      mangaInfo: App.createMangaInfo({
        ...
        desc: description,
        status,
        tags: tags.length > 0 ? [App.createTagSection({ id: "genres", label: "Genres", tags })] : []
      })
    });
  };
  var MONTHS = [
  ...
  var parseChapters = ($2, mangaId) => {
    const chapters = [];
    const seen = /* @__PURE__ */ new Set();
    for (const element of $2("li.wp-manga-chapter, div.chapter-item").toArray()) {
      const anchor = $2(element).find('a[href*="/manga/"]').first();
...
Please go to the homepage of <${ManhwaClubAllInfo.name}> and press the cloud icon.`);
    ...
    async getSearchResults(query, metadata) {
      ...
      const url = genre != void 0 && !query.title ? `${MC_DOMAIN}/manga-genre/${genre}/page/${page}/` : `${MC_DOMAIN}/page/${page}/?s=${encodeURIComponent(query.title ?? "")}&post_type=wp-manga${genre != void 0 ? `&genre[]=${encodeURIComponent(genre)}` : ""}`;
  ...
  return __toCommonJS(ManhwaClubAll_exports);
})();
