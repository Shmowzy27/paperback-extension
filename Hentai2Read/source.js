...
var _Sources = (() => {
  ...
  var Hentai2Read_exports = {};
  __export(Hentai2Read_exports, {
    H2R_DOMAIN: () => H2R_DOMAIN,
    Hentai2Read: () => Hentai2Read,
    Hentai2ReadInfo: () => Hentai2ReadInfo
  });
  ...
  var GROUP_RULE = { group: ["311"], oneMale: ["31", "1686"], oneFemale: [] };
...
Please go to the homepage of <${Hentai2ReadInfo.name}> and press the cloud icon.`);
    ...
    async fetchHtml(url, method = "GET", body) {
      const request = App.createRequest({
        url,
        method,
        data: body,
        headers: body != void 0 ? { "content-type": "application/x-www-form-urlencoded" } : void 0
      });
    ...
    parseTiles(html3, seen, filters2) {
      ...
      for (const element of $2('a[href^="https://hentai2read.com/"]').toArray()) {
    ...
    async getMangaDetails(mangaId) {
      ...
      const description = ($2('meta[property="og:description"]').attr("content") ?? "").trim();
      const author = $2('ul.list-simple-mini a.tagButton[href*="/hentai-list/artist/"], ul.list-simple-mini a.tagButton[href*="/hentai-list/author/"]').toArray().map((element) => $2(element).text().trim()).filter((name, index2, list) => name.length > 0 && list.indexOf(name) === index2).join(", ");
      const tags = [];
      ...
      const status = /\/hentai-list\/status\/Completed\//.test($2("ul.list-simple-mini").html() ?? "") ? "Completed" : "Ongoing";
      return App.createSourceManga({
        ...
        mangaInfo: App.createMangaInfo({
          ...
          status,
          author,
          tags: tags.length > 0 ? [App.createTagSection({ id: "category", label: "Categories", tags })] : []
        })
      });
    }
    async getChapters(mangaId) {
      ...
      const rows = [];
      const seen = /* @__PURE__ */ new Set();
      for (const element of $2(`a[href^="https://hentai2read.com/${mangaId}/"]`).toArray()) {
        const anchor = $2(element);
      ...
      return rows.map((row, index2) => App.createChapter({
        id: row.slug,
        chapNum: isNaN(row.number) ? index2 + 1 : row.number,
        name: row.name,
    ...
    async getChapterDetails(mangaId, chapterId) {
      const html3 = await this.fetchHtml(`${H2R_DOMAIN}/${mangaId}/${chapterId}/`);
      const body = /['"]images['"]\s*:\s*\[([^\]]*)\]/.exec(html3)?.[1] ?? "";
      const pages = [];
     ...
     */
    async getSearchTags() {
      const $2 = load(await this.fetchHtml(`${H2R_DOMAIN}/hentai-list/advanced-search/`));
      const tags = [];
      const seen = /* @__PURE__ */ new Set();
      for (const element of $2('a[href*="/hentai-list/category/"]').toArray()) {
        const slug = /\/hentai-list\/category\/([^/"]+)/.exec($2(element).attr("href") ?? "")?.[1];
        const label = $2(element).text().trim();
        if (slug == void 0 || label.length === 0 || seen.has(slug)) continue;
    ...
    async getHomePageSections(sectionCallback) {
      for (const entry of SECTIONS) {
        ...
        const html3 = await this.fetchHtml(`${H2R_DOMAIN}${entry.path(1)}`);
        section.items = this.parseTiles(html3, /* @__PURE__ */ new Set());
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const html3 = await this.fetchHtml(`${H2R_DOMAIN}${entry.path(page)}`);
  ...
  return __toCommonJS(Hentai2Read_exports);
})();
